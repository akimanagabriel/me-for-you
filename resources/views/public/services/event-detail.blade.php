    ...
    <div class="container mx-auto px-4">
        <nav class="text-sm text-[#7a7268]">
            <ol class="flex items-center space-x-2">
                ...
                <li><a href="{{ route('services.events') }}" class="hover:text-[#b87f3a] transition-colors">Events</a></li>
                <li><i class="fas fa-chevron-right text-xs text-[#7a7268]/40"></i></li>
    ...
    <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-2">
                ...
                <!-- Title & Basic Info -->
                <div class="mb-6">
                    <h1 class="font-display text-3xl md:text-4xl font-semibold text-[#1a1714] mb-3">{{ $event->title }}</h1>
                            ...
                            @if($event->price)
                                <p class="font-display text-3xl font-semibold text-[#b87f3a]">
                                    ...
                                    @if($event->price_period !== 'total')
                                        <span class="text-sm font-normal text-[#7a7268]">/ {{ str_replace('_', ' ', $event->price_period) }}</span>
                                    @endif
                                </p>
                            @else
                        ...
                        <div class="bg-[#f5f0e8] rounded-xl p-4">
                            @if($event->start_time)
                                <div class="flex items-center gap-4 py-2">
                                    <div class="w-12 h-12 bg-[#b87f3a]/10 rounded-lg flex items-center justify-center text-[#b87f3a]">
                                        <i class="fas fa-play"></i>
                                    </div>
                                    <div>
                                        <p class="text-xs text-[#7a7268]">Start Time</p>
                                        <p class="font-medium text-[#1a1714]">{{ \Carbon\Carbon::parse($event->start_time)->format('h:i A') }}</p>
                                    </div>
                                </div>
                            @endif
                            @if($event->end_time)
                                <div class="flex items-center gap-4 py-2 border-t border-[#ede7d8]">
                                    ...
                                    <div>
                                        ...
                                        <p class="font-medium text-[#1a1714]">{{ \Carbon\Carbon::parse($event->end_time)->format('h:i A') }}</p>
                                    </div>
                                </div>
                            @endif
                        </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                            @foreach($event->images as $image)
                                <div class="rounded-lg overflow-hidden aspect-square bg-[#f5f0e8] hover:opacity-90 transition-opacity cursor-pointer" onclick="openLightbox('{{ $image->image_path }}', '{{ $image->alt_text ?? $event->title }}')">
            ...
            <div class="space-y-6">
                            ...
                            <div>
                                ...
                                @if($event->price)
                                    <p class="font-medium text-[#1a1714] text-lg">
                                        {{ number_format($event->price, 0, ',', ',') }} {{ $event->currency }}
                                        @if($event->price_period !== 'total')
                                            <span class="text-xs text-[#7a7268] font-normal">/ {{ str_replace('_', ' ', $event->price_period) }}</span>
                                        @endif
                                    </p>
                                @else
                        ...
                        <div class="space-y-3 text-sm">
                            @if($event->speaker)
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <p class="text-xs text-[#7a7268] font-medium uppercase tracking-wider">Speaker</p>
                            ...
                            @if($event->organizer)
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <p class="text-xs text-[#7a7268] font-medium uppercase tracking-wider">Organizer</p>
                        ...
                        <div class="space-y-3 text-sm">
                            @if($event->contact_email)
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <p class="text-xs text-[#7a7268] font-medium uppercase tracking-wider">Email</p>
                                        <a href="mailto:{{ $event->contact_email }}" class="text-[#b87f3a] font-medium hover:underline">{{ $event->contact_email }}</a>
                                    </div>
                                </div>
                            @endif
                            @if($event->contact_phone)
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <p class="text-xs text-[#7a7268] font-medium uppercase tracking-wider">Phone</p>
                                        <a href="tel:{{ $event->contact_phone }}" class="text-[#b87f3a] font-medium hover:underline">{{ $event->contact_phone }}</a>
                                    </div>
                                </div>
                            @endif
                        </div>
                ...
                <div class="bg-[#f5f0e8] rounded-xl p-6 border border-[#ede7d8]">
                    <h3 class="font-display text-xl font-semibold text-[#1a1714] mb-4 flex items-center gap-2">
                        <i class="fas fa-share-alt text-[#b87f3a]"></i> Share This Event
                    </h3>
                    <div class="flex gap-3">
                        ...
                        <a href="https://twitter.com/intent/tweet?text={{ urlencode($event->title) }}&url={{ urlencode(url()->current()) }}" target="_blank" class="w-10 h-10 bg-[#000000] text-white rounded-full flex items-center justify-center hover:bg-[#1a1a1a] transition-colors">
                            <i class="fab fa-x-twitter"></i>
                        </a>
                        <a href="https://wa.me/?text={{ urlencode($event->title . ' - ' . url()->current()) }}" target="_blank" class="w-10 h-10 bg-[#25D366] text-white rounded-full flex items-center justify-center hover:bg-[#1da851] transition-colors">
                            <i class="fab fa-whatsapp"></i>
                        </a>
                        <a href="mailto:?subject={{ urlencode($event->title) }}&body={{ urlencode('Check out this event: ' . url()->current()) }}" class="w-10 h-10 bg-[#7a7268] text-white rounded-full flex items-center justify-center hover:bg-[#6a6258] transition-colors">
                            <i class="fas fa-envelope"></i>
                        </a>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    @endif
                    <a href="{{ route('services.events') }}" class="block w-full text-center border border-[#ede7d8] text-[#7a7268] font-semibold px-6 py-3 rounded-lg hover:bg-[#f5f0e8] transition-colors">
        ...
        @if($relatedEvents->isNotEmpty())
            <div class="mt-16 pt-10 border-t border-[#ede7d8]">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('services.events') }}" class="text-[#b87f3a] font-semibold hover:underline inline-flex items-center gap-1">
                    ...
                    @foreach($relatedEvents as $related)
                        <div class="group bg-[#f5f0e8] rounded-xl overflow-hidden hover:shadow-md transition-shadow duration-300">
                            <div class="relative h-40 overflow-hidden">
                                <img src="{{ $related->cover_image ?? asset('assets/events/event-placeholder.jpg') }}" alt="{{ $related->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                @if($related->status === 'active')
                                    <span class="absolute top-2 right-2 bg-green-500 text-white text-[10px] font-semibold px-2 py-0.5 rounded-full">Active</span>
                                @elseif($related->status === 'completed')
                                    <span class="absolute top-2 right-2 bg-[#7a7268] text-white text-[10px] font-semibold px-2 py-0.5 rounded-full">Completed</span>
                                @endif
                            </div>
                            <div class="p-4">
                                ...
                                <a href="{{ route('services.event.show', $related->slug) }}" class="mt-2 inline-flex items-center gap-1 text-[#b87f3a] text-sm font-semibold hover:underline group-hover:gap-2 transition-all">

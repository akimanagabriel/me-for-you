    ...
    <div class="container mx-auto px-4">
        <nav class="text-sm text-[#7a7268]">
            <ol class="flex items-center space-x-2">
                ...
                <li><a href="{{ route('services.transport') }}" class="hover:text-[#b87f3a] transition-colors">Transport</a></li>
                <li><i class="fas fa-chevron-right text-xs text-[#7a7268]/40"></i></li>
    ...
    <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-2">
                ...
                <!-- Title & Basic Info -->
                <div class="mb-6">
                    <h1 class="font-display text-3xl md:text-4xl font-semibold text-[#1a1714] mb-3">{{ $car->title }}</h1>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        ...
                        <div class="flex items-center gap-3 p-3 bg-[#f5f0e8] rounded-lg">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-[#1a1714]">{{ ucwords(str_replace('_', ' ', $car->condition)) }}</p>
                            </div>
                        </div>
                        @if($car->vin)
                            <div class="flex items-center gap-3 p-3 bg-[#f5f0e8] rounded-lg">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                            @foreach($car->images as $image)
                                <div class="rounded-lg overflow-hidden aspect-square bg-[#f5f0e8] hover:opacity-90 transition-opacity cursor-pointer" onclick="openLightbox('{{ $image->image_path }}', '{{ $image->alt_text ?? $car->title }}')">
                                    <img src="{{ $image->image_path }}" alt="{{ $image->alt_text ?? $car->title }}" class="w-full h-full object-cover">
                                    @if($image->is_cover)
            ...
            <div class="space-y-6">
                ...
                <div class="bg-[#f5f0e8] rounded-xl p-6 border border-[#ede7d8]">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center gap-3 p-3 bg-white rounded-lg">
                            <div class="w-12 h-12 bg-[#b87f3a]/10 rounded-full flex items-center justify-center text-[#b87f3a] text-xl font-bold">
                                ME
                            </div>
                            <div>
                        ...
                        <a href="tel:+250788202209" class="flex items-center gap-3 p-3 bg-white rounded-lg hover:bg-[#f5f0e8] transition-colors">
                            <i class="fas fa-phone text-[#b87f3a]"></i>
                            <span class="text-[#1a1714]">+250 788 202 209</span>
                        </a>
                        <a href="mailto:info@me-for-you.org" class="flex items-center gap-3 p-3 bg-white rounded-lg hover:bg-[#f5f0e8] transition-colors">
                            <i class="fas fa-envelope text-[#b87f3a]"></i>
                            <span class="text-[#1a1714]">info@me-for-you.org</span>
                        </a>
                        <a href="https://wa.me/+250788202209" target="_blank" class="flex items-center gap-3 p-3 bg-white rounded-lg hover:bg-[#f5f0e8] transition-colors">
                            <i class="fab fa-whatsapp text-[#25D366]"></i>
                            <span class="text-[#1a1714]">Chat on WhatsApp</span>
                        </a>
                    </div>
                    <a href="{{ route('contact') }}" class="mt-4 block w-full text-center bg-[#b87f3a] text-white font-semibold px-6 py-3 rounded-lg hover:bg-[#8a6e22] transition-colors">
                ...
                <div class="bg-[#f5f0e8] rounded-xl p-6 border border-[#ede7d8]">
                    <h3 class="font-display text-xl font-semibold text-[#1a1714] mb-4 flex items-center gap-2">
                        <i class="fas fa-share-alt text-[#b87f3a]"></i> Share This Vehicle
                    </h3>
                    <div class="flex gap-3">
                        ...
                        <a href="https://twitter.com/intent/tweet?text={{ urlencode($car->title) }}&url={{ urlencode(url()->current()) }}" target="_blank" class="w-10 h-10 bg-[#000000] text-white rounded-full flex items-center justify-center hover:bg-[#1a1a1a] transition-colors">
                            <i class="fab fa-x-twitter"></i>
                        </a>
                        <a href="https://wa.me/?text={{ urlencode($car->title . ' - ' . url()->current()) }}" target="_blank" class="w-10 h-10 bg-[#25D366] text-white rounded-full flex items-center justify-center hover:bg-[#1da851] transition-colors">
                            <i class="fab fa-whatsapp"></i>
                        </a>
                        <a href="mailto:?subject={{ urlencode($car->title) }}&body={{ urlencode('Check out this vehicle: ' . url()->current()) }}" class="w-10 h-10 bg-[#7a7268] text-white rounded-full flex items-center justify-center hover:bg-[#6a6258] transition-colors">
                ...
                <div class="space-y-3">
                    ...
                    @endif
                    <a href="{{ route('services.transport') }}" class="block w-full text-center border border-[#ede7d8] text-[#7a7268] font-semibold px-6 py-3 rounded-lg hover:bg-[#f5f0e8] transition-colors">
        ...
        @if($relatedCars->isNotEmpty())
            <div class="mt-16 pt-10 border-t border-[#ede7d8]">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('services.transport') }}" class="text-[#b87f3a] font-semibold hover:underline inline-flex items-center gap-1">
                    ...
                    @foreach($relatedCars as $related)
                        <div class="group bg-[#f5f0e8] rounded-xl overflow-hidden hover:shadow-md transition-shadow duration-300">
                            <div class="relative h-48 overflow-hidden">
                                <img src="{{ $related->cover_image ?? asset('assets/transport/car-placeholder.jpg') }}" alt="{{ $related->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                ...
                                @elseif($related->status === 'reserved')
                                    <span class="absolute top-2 right-2 bg-yellow-500 text-white text-[10px] font-semibold px-2 py-0.5 rounded-full">Reserved</span>
                                @endif
                            </div>
                            <div class="p-4">
                                ...
                                <div class="flex items-center justify-between mt-2">
                                    ...
                                    <a href="{{ route('services.car.show', $related->slug) }}" class="text-[#b87f3a] text-sm font-semibold hover:underline inline-flex items-center gap-1 group-hover:gap-2 transition-all">

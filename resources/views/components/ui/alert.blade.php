...
<div {{ $attributes->merge(['role' => 'alert', 'class' => 'alert ' . ($types[$type] ?? 'alert-info')]) }}>
    @switch($type)
        @case('success')
    ...
    @if ($dismissible)
        <button type="button" class="btn btn-sm btn-ghost btn-circle" onclick="this.closest('.alert').remove()">✕</button>
    @endif
</div>

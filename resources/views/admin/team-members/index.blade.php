...
@section('content')
    ...
    <div class="bg-base-100 rounded-xl shadow-sm border border-base-300 p-4 mb-6">
        <form method="GET" action="{{ route('admin.team-members.index') }}" class="flex flex-wrap items-center gap-3">
            <div class="flex-1 min-w-[200px]">
                <input type="text" name="search" placeholder="Search by name, position, department..."
                    value="{{ request('search') }}" class="input input-bordered input-sm w-full">
            </div>
            <div class="w-[140px]">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($members as $member)
            <div
                class="bg-base-100 rounded-xl shadow-sm border border-base-300 overflow-hidden hover:shadow-md transition-shadow duration-200 flex flex-col">
                ...
                <div class="flex items-start gap-4 p-4 pb-2">
                    <div class="w-16 h-16 rounded-full bg-base-200 flex-shrink-0 overflow-hidden border-2 border-base-300">
                        ...
                        @else
                            <div
                                class="w-full h-full flex items-center justify-center text-base-content/30 text-2xl font-bold">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                <h3 class="font-display text-lg font-semibold text-base-content truncate">
                                    {{ $member->name }}</h3>
                            ...
                            <div class="flex-shrink-0 mt-0.5 flex items-center gap-1">
                                <span
                                    class="badge badge-soft {{ $member->is_active ? 'badge-success' : 'badge-error' }} text-xs">
                ...
                <div class="px-4 py-3 border-t border-base-200 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <form method="POST" action="{{ route('admin.team-members.toggle', $member) }}"
                            class="inline-flex items-center gap-2" id="toggle-form-{{ $member->id }}">
                            @csrf
                            @method('PUT')
                            <span
                                class="text-xs text-base-content/40 {{ $member->is_active ? 'text-success' : 'text-error' }} font-medium">
                                {{ $member->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        ...
                        <form method="POST" action="{{ route('admin.team-members.toggle-featured', $member) }}"
                            class="inline-flex items-center gap-2" id="toggle-featured-form-{{ $member->id }}">
                            @csrf
                            @method('PUT')
                            <span
                                class="text-xs text-base-content/40 {{ $member->is_featured ? 'text-accent' : '' }} font-medium">
                                <i class="fas fa-star mr-0.5"></i> Featured
                            </span>
                            <label class="cursor-pointer">
                    ...
                    <div class="flex items-center gap-1">
                        <a href="{{ route('admin.team-members.edit', $member) }}"
                            class="btn btn-ghost btn-xs btn-square text-primary" title="Edit">
                        ...
                        <form id="delete-form-{{ $member->id }}"
                            action="{{ route('admin.team-members.destroy', $member) }}" method="POST" class="hidden">
                            @csrf @method('DELETE')
                        </form>
                    </div>
                </div>
